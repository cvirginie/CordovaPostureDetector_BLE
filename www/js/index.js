...
onData: function(data) { // data received from Arduino
    ...
    if(bytesToString(data) != "warning" && bytesToString(data) != "wrong")
    {
        var value = parseFloat(bytesToString(data));

        if(localStorage.getItem("ok") == 1){
            localStorage.setItem("ok", value);
        }

        if(bytesToString(data).substring(0, 4) == "back" ){
            localStorage.setItem("ok", 1);
        }
        ...
        ctx.beginPath(); // commencer à dessiner
        ctx.moveTo(localStorage.getItem("ok"),localStorage.getItem("ok"));
        ctx.lineTo(150,value);
        ctx.stroke();
    }
},
